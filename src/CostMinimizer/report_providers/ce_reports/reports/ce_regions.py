...
class CeRegions(CeBase):
	...
	def description(self): #required - see abstract class
		return '''Montly CostExplorer Regions View.'''
	
	def long_description(self):
		return f'''CostExplorer Regions View:
		This report breaks down AWS costs by geographic regions.
		It provides insights into:
		- Total spend per AWS region
		- Percentage of overall costs for each region
		- Trends in regional cost distribution over time
		Use this view to optimize costs by identifying high-cost regions and making informed decisions about resource placement and data transfer.'''
	...
	def sql(self, replace=True, query_type='sql_s_r'): #required - see abstract class
		return { "Name":"Regions", "GroupBy": [{"Type": "DIMENSION","Key": "REGION"}], "Style": "Total" }
