...
class CeServices(CeBase):
	...
	def description(self): #required - see abstract class
		return '''Montly CostExplorer Services View.'''
	
	def long_description(self) -> str:
		return f'''CostExplorer Services View:
		This report breaks down AWS costs by individual services.
		It provides insights into:
		- Total spend per AWS service (e.g., EC2, S3, RDS)
		- Percentage of overall costs for each service
		- Trends in service usage and costs over time
		Use this view to identify high-cost services, optimize service usage, and make informed decisions about architectural choices and cost allocation.'''
	...
	def sql(self, replace=True, query_type='sql_s_r'): #required - see abstract class
		return { "Name":"Services", "GroupBy": [{"Type": "DIMENSION","Key": "SERVICE"}], "Style": "Total", "IncSupport":True }
