...
class CeAccounts(CeBase):
	...
	def description(self): #required - see abstract class
		return '''Montly CostExplorer Accounts Total View.'''
	
	def long_description(self):
		return f'''CostExplorer Accounts Total View:
		This report provides a comprehensive overview of AWS account costs.
		It displays total spend for each account over the specified lookback period (1-12 months).
		Use this view to:
		- Track and analyze account-level spending trends over time
		- Identify high-spending accounts for further investigation
		- Compare costs across different AWS accounts in your organization'''
	...
	def sql(self, replace=True, query_type='sql_s_r'): #required - see abstract class
		return { "Name":"Accounts", "GroupBy": [{"Type": "DIMENSION","Key": "LINKED_ACCOUNT"}], "Style": "Total" }
