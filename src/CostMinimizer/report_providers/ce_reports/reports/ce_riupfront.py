...
class CeRiupfront(CeBase):
	...
	def description(self): #required - see abstract class
		return '''Montly CostExplorer Reserved Instance (RI) Upfronts View.'''
	
	def long_description(self) -> str:
		return f'''CostExplorer Reserved Instance (RI) Upfronts View:
		This report focuses on the upfront costs associated with RI purchases.
		It provides details on:
		- Total upfront RI costs over time
		- Breakdown of upfront costs by instance type and term
		- Amortization of upfront costs over the RI term
		Use this view to understand the initial investment required for RIs and plan your RI purchase strategy accordingly.'''
	...
	def sql(self, replace=True, query_type='sql_s_r'): #required - see abstract class
		return { "Name":"RIUpfront", "GroupBy": [], "Style": "Total", "UpfrontOnly": True }
