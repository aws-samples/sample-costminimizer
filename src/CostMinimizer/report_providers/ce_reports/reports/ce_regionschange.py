...
class CeRegionschange(CeBase):
	...
	def name(self): #required - see abstract class
		return 'ce_regionschange'

	def long_name(self):
		return CeBase.long_name()

	def common_name(self) -> str:
		return 'REGIONS CHANGES view'
	...
	def description(self): #required - see abstract class
		return '''Montly CostExplorer Regions Change View.'''
	
	def long_description(self):
		return f'''CostExplorer Regions Change View:
		This report focuses on cost changes across AWS regions over time.
		Unlike the regular Regions view, this report highlights:
		- Month-over-month cost variations for each region
		- Percentage changes in regional spend
		- Identification of regions with significant cost increases or decreases
		Use this view to quickly identify shifting cost patterns across different geographic areas and inform multi-region deployment strategies.'''
	...
	def sql(self, replace=True, query_type='sql_s_r'): #required - see abstract class
		return { "Name":"RegionsChange", "GroupBy": [{"Type": "DIMENSION","Key": "REGION"}], "Style": "Change" }
