...
class CeAccountschange(CeBase):
	...
	def name(self): #required - see abstract class
		return 'ce_accountschange'

	def long_name(self):
		return CeBase.long_name()

	def common_name(self) -> str:
		return 'CHANGE /ACCOUNTS view'
	...
	def description(self): #required - see abstract class
		return '''Montly CostExplorer Accounts Change View.'''
	
	def long_description(self) -> str:
		return f'''CostExplorer Accounts Change View:
		This report focuses on cost changes across AWS accounts over time.
		Unlike the regular Accounts view, this report highlights:
		- Month-over-month cost variations for each account
		- Percentage changes in spend
		- Identification of accounts with significant cost increases or decreases
		Use this view to quickly spot trends and anomalies in account spending patterns.'''
	...
	def sql(self, replace=True, query_type='sql_s_r'): #required - see abstract class
		return { "Name":"AccountsChange", "GroupBy": [{"Type": "DIMENSION","Key": "LINKED_ACCOUNT"}], "Style": "Change" }
