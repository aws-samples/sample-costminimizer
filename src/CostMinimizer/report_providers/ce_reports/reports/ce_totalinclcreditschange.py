...
class CeTotalinclcreditschange(CeBase):
	...
	def name(self): #required - see abstract class
		return 'ce_totalinclcreditschange'

	def long_name(self):
		return CeBase.long_name()

	def common_name(self) -> str:
		return 'CREDITS CHANGES view'
	...
	def description(self): #required - see abstract class
		return '''Montly CostExplorer Total Spend changes incl credits View.'''
	...
	def sql(self, replace=True, query_type='sql_s_r'): #required - see abstract class
		return { "Name":"TotalInclCreditsChange", "GroupBy": [], "Style": "Change", "NoCredits": False }
