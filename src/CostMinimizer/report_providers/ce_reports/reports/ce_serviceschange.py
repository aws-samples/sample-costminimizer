...
class CeServiceschange(CeBase):
	...
	def name(self): #required - see abstract class
		return 'ce_serviceschange'

	def long_name(self):
		return CeBase.long_name()

	def common_name(self) -> str:
		return 'SERVICES CHANGES view'
	...
	def description(self): #required - see abstract class
		return '''Montly CostExplorer Services Change View.'''
	
	def long_description(self) -> str:
		return f'''CostExplorer Services Change View:
		This report focuses on cost changes across AWS services over time.
		Unlike the regular Services view, this report highlights:
		- Month-over-month cost variations for each AWS service
		- Percentage changes in service spend
		- Identification of services with significant cost increases or decreases
		Use this view to quickly spot trends, detect unusual service usage patterns, and identify opportunities for cost optimization across your AWS service portfolio.'''
	...
	def sql(self, replace=True, query_type='sql_s_r'): #required - see abstract class
		return { "Name":"ServicesChange", "GroupBy": [{"Type": "DIMENSION","Key": "SERVICE"}], "Style": "Change" }
