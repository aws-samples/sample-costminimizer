...
class CeRiutilizationsavings(CeBase):
	...
	def name(self): #required - see abstract class
		return 'ce_riutilizationsavings'

	def long_name(self):
		return CeBase.long_name()

	def common_name(self) -> str:
		return 'RI UTILIZATION SAVINGS view'
	...
	def description(self): #required - see abstract class
		return '''Montly CostExplorer Reserved Instance (RI) Utilization Savings View.'''
	
	def long_description(self) -> str:
		return f'''CostExplorer Reserved Instance (RI) Utilization Savings View:
		This report quantifies the cost savings achieved through RI utilization.
		It provides:
		- Total cost savings from RI usage compared to On-Demand pricing
		- Breakdown of savings by instance type and region
		- Trends in RI savings over time
		Use this view to demonstrate the financial benefits of your RI investments and identify areas where additional savings can be realized through improved RI strategies.'''
	...
	def sql(self, replace=True, query_type='sql_s_r'): #required - see abstract class
		return { "Name":"RIUtilizationSavings", "Savings":True }
