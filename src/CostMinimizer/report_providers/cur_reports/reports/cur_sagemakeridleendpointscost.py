...
class CurSagemakeridleendpointscost(CurBase):
    """
    A class for identifying and reporting on potential cost savings by optimizing idle SageMaker endpoints in AWS environments.
    
    This class extends CurBase and provides methods for analyzing Cost and Usage Report (CUR) data
    to identify SageMaker endpoints that are idle or underutilized, potentially leading to unnecessary costs.
    """

    def name(self):
        return "cur_sagemakeridleendpointscost"

    def common_name(self):
        return "SageMaker Idle Endpoints Cost"
    ...
    def description(self):
        return "Identifies costs associated with idle SageMaker endpoints"

    def long_description(self):
        return f'''This check identifies idle SageMaker endpoints in your AWS environment, helping you optimize costs and resource usage.
        By pinpointing underutilized endpoints, it enables you to make informed decisions about your machine learning infrastructure.
        An idle SageMaker endpoint is defined as one that has little to no inference activity over an extended period. 
        These endpoints continue to incur charges even when not actively used, leading to unnecessary costs and resource inefficiency.
        Potential Savings:
        - Direct Cost Reduction: Stopping or removing idle SageMaker endpoints can lead to immediate and substantial savings on your AWS bill.
        ...
        - Scalable Impact: The more idle endpoints identified and addressed, the greater the potential savings, especially in environments with extensive SageMaker usage.'''
    ...
    def _set_recommendation(self):
        self.recommendation = f'''Returned {self.count_rows()} rows summarizing potential cost savings from optimizing idle SageMaker endpoints.'''
        ...
        if len(response) == 0:
            ...
            for resource in response[1:]:
                data_dict = {
                    ...
                    self.get_required_columns()[2]: resource['Data'][1]['VarCharValue']
                }
    ...
    def sql(self, fqdb_name: str, payer_id: str, account_id: str, region: str):
        # This method needs to be implemented with the specific SQL query for SageMaker idle endpoints cost
...
AND line_item_usage_start_date >= now() - INTERVAL '15' DAY 
