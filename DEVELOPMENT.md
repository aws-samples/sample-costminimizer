...
This is a [Python 3] Python project.
...
`python setup.py develop` will run the develop command defined in `setup.py`. 
The arguments for this will be taken from `setup.cfg`'s `[tool:pytest]` section, 
but can be set in `pytest.ini` if that's your thing too. Coverage is enabled by default, 
provided by pytest-cov, which is part of the `PythonTestingDependencies` package group.

## Running

To run, you just do this:

`--help`

## Deploying

If this is a library, nothing needs to be done; it'll deploy the versions it builds. 
